...
from utils.WIoU_scale import WIoU_Scale


def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, WIoU=False,
    ...
    if GIoU or DIoU or CIoU or EIoU or SIoU or WIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU or WIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if DIoU:
                ...
                if scale:
                    return getattr(WIoU_Scale, '_scaled_loss')(self), (1 - iou) * torch.exp((rho2 / c2)), iou  # Scale * WIoU v1
                    # WIoU https://arxiv.org/abs/2301.10051
                else:
                    return iou, torch.exp((rho2 / c2))  # WIoU v1  (1-IOU) * torch.exp((rho2 / c2))
            else:  # EIoU
...
def bbox_alpha_iou(box1, box2, x1y1x2y2=False, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False,
                   WIoU=False, scale=False, alpha=2, eps=1e-9):
    ...
    if GIoU or DIoU or CIoU or SIoU or EIoU:
        ...
        if CIoU or DIoU or SIoU or EIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if DIoU:
                ...
                omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)
                shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
                return iou - (torch.pow(0.5 * distance_cost, alpha) + torch.pow(0.5 * shape_cost, alpha))
            elif WIoU:
                # if Focal:
                #     raise RuntimeError("WIoU do not support Focal.")
                if scale:
                ...
                cw2 = torch.pow(cw, 2) + eps
                ch2 = torch.pow(ch, 2) + eps
                return iou - (rho2 / c2 + torch.pow(w_dis / cw2, alpha) + torch.pow(h_dis / ch2, alpha))
        else:  # GIoU https://arxiv.org/pdf/1902.09630.pdf
            # c_area = cw * ch + eps  # convex area
            # return iou - (c_area - union) / c_area  # GIoU
...
def strip_optimizer(f='yolov7-tiny_MobileTiny_backbone_b2_neck_W_noAttention2.pt', s=''):  # from utils.general import *; strip_optimizer()
