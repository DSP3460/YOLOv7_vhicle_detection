    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # iou = bbox_alpha_iou(pbox.T, tbox[i], x1y1x2y2=False, CIoU=True)  # use alpha CIoU
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, SIoU=True)  # use SIoU
                # iou = bbox_alpha_iou(pbox.T, tbox[i], x1y1x2y2=False, SIoU=True)  # use alpha SIoU
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, EIoU=True)  # use EIoU
                lbox += (1.0 - iou).mean()  # iou loss
                # wiou v1
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, WIoU=True)
                # lbox += (iou[1].detach().squeeze() * (1-iou[0].squeeze())).mean()  # wiou v1 iou loss
                # iou = iou[0].squeeze()
                # wiou v2 or v3   adjust WIoU_scale.py value monotonous
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, WIoU=True, scale=True)
                # lbox += (iou[1] * iou[0]).mean()
                # iou = iou[2].squeeze()
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # iou = bbox_alpha_iou(pbox.T, selected_tbox, x1y1x2y2=False, CIoU=True)  # use alpha CIoU
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, SIoU=True)  # use SIoU
                # iou = bbox_alpha_iou(pbox.T, selected_tbox, x1y1x2y2=False, SIoU=True)  # use alpha SIoU
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, EIoU=True)  # use EIoU
                lbox += (1.0 - iou).mean()  # iou loss
                # wiou v1
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, WIoU=True)
                # lbox += (iou[1].detach().squeeze() * (1 - iou[0].squeeze())).mean()  # wiou v1 iou loss
                # iou = iou[0].squeeze()
                # wiou v2 or v3   adjust WIoU_scale.py value monotonous
                # iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, WIoU=True, scale=True)
                # lbox += (iou[1] * iou[0]).mean()
                # iou = iou[2].squeeze()
