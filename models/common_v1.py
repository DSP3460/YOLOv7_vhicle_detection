...
class GAMAttention(nn.Module):
    ...
    def __init__(self, c1, c2, group=True, rate=4):
        ...
        self.channel_attention = nn.Sequential(
            nn.Linear(c1, int(c1 / rate)),  # 2 layer MLP
...
def INF(B, H, W, device):
    # generate (b*w, h, h)
    # add para device to ensurance data and model in same GPU
...
#
#
# @parse_args("v", "i", "none", "none")
# def symbolic(g, input, diagonal, *, out):
#     return g.op("custom::diag", input, diagonal)
#
#
# register_custom_op_symbolic("torch::diag", symbolic, 12)
# from torch.ONNX.symbolic_registry import register_op
#
#
# def diag_symbolic(g, input, diagonal, *, out=None):
#     return g.op("diag", input, diagonal)
#
#
# register_op("diag", diag_symbolic, '', 16)
...
class CrissCrossAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Q shuffle channel
        proj_query = channel_shuffle(proj_query, 4)
        ...
        # K shuffle channel
        proj_key = channel_shuffle(proj_key, 4)
        ...
        # V shuffle channel
        proj_value = channel_shuffle(proj_value, 4)
        # (b, c, h, w) => (b, w, c, h) => (b*w, c, h)
        ...
        # affinity
        # (b*w, h, h)+(b*w, h, h) => (b, w, h, h) => (b, h, w, h)
        # energy_H = (torch.bmm(proj_query_H, proj_key_H) + self.INF(m_batchsize, height, width, x.device))
        ...
        concate = self.softmax(torch.cat([energy_H, energy_W], 3))  # Softmax
        ...
        # (b*w, c, h)*(b*w, h, h) => (b*w, c, h) => (b, w, c, h) => (b, c, h, w)
        out_H = torch.bmm(proj_value_H, att_H.permute(0, 2, 1)).view(m_batchsize, width, -1, height).permute(0, 2, 3, 1)
        # shuffle channel H
        # (b, c, h, w) => (b, c//4, 4, h, w) => (b, c, h, w)
        out_H = channel_shuffle(out_H, 4)
        # (b*h, c, w) * (b*h, w, w) = > (b*h, c, w) = > (b, h, c, w) = > (b, c, h, w)
        out_W = torch.bmm(proj_value_W, att_W.permute(0, 2, 1)).view(m_batchsize, height, -1, width).permute(0, 2, 1, 3)
        # shuffle channel W
        out_W = channel_shuffle(out_W, 4)
...
class DS_Conv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        # self.conv = nn.Conv2d(c1, c1, k, s, autopad(k, p), groups=g, bias=False)
        self.conv1 = nn.Conv2d(c1, c1, k, s, autopad(k, p), groups=c1, bias=False)
        self.conv2 = nn.Conv2d(c1, c2, 1, s, autopad(1, p), groups=1)
...
class Res_Bottleneck(nn.Module):
    """
        残差组件 = DSConv1 -> DSConv2 -> (inputs + DSConv2)
    """

    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = DS_Conv(c1, c_, 1, 1)
        self.cv2 = DS_Conv(c_, c2, 3, 1, g=g)
...
# ####  MobileTiny  ####
class MobileTiny(nn.Module):
    """
        自定义的MobileTiny结构，借鉴CSP、C3、Resnet、DSConv
    """

    def __init__(self, c1, c2, n=1, shortcut=True, g=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.bn = nn.BatchNorm2d(c2)  # (B, C, H, W)
        self.m = nn.Sequential(*(Res_Bottleneck(c2, c2, shortcut, g, e=1.0) for _ in range(n)))  # (B, C, H, W)
        # self.cv = DS_Conv(c2, c2, 1)  # optional act=SiLU(c2) (B, C, H, W)

    def forward(self, x):
        out_add = self.bn(x) + self.m(x)  # (B, C, H, W)+(B, C, H, W) = (B, C, H, W)
        return out_add
...
# #### DeepWise Separable Conv BN ACT ####
class DS_CBL(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, act=True):  # ch_in, ch_out, kernel, stride, padding, act
        super(DS_CBL, self).__init__()
        self.k = k
        self.conv1 = nn.Conv2d(c1, c1, k, s, autopad(k, p), groups=c1, bias=False)
        self.conv2 = nn.Conv2d(c1, c2, 1, s, autopad(1, p), groups=1)
        self.bn = nn.BatchNorm2d(c2)
...
# #### DeepWise Conv BN ACT end  ####


# ####  Res Bottleneck  ####
class Res_DS_CBL(nn.Module):
    """
        残差组件 = DSCBL1 -> DSCBL2 -> (inputs + DSCBL2)
    """

    def __init__(self, c1, c2, k=1, s=1, shortcut=True, e=0.5, act=True):  # ch_in, ch_out, k, s, shortcut, e, act
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = DS_CBL(c1, c_, 1, 1, None, act)
        self.cv2 = DS_CBL(c_, c2, k, s, None, act)
        self.add = shortcut and c1 == c2
...
# ####  MobileTiny  ####
class MobileTiny_base(nn.Module):
    """
        自定义的MobileTiny_base结构，借鉴CSP、C3、Resnet、DSConv
    """

    def __init__(self, c1, c2, shortcut=True):  # ch_in, ch_out, shortcut, groups
        super().__init__()
        c_ = int(c2 * 0.25)
        self.b1 = Res_DS_CBL(c1, c_, 1, 1, shortcut, 0.5)
        self.b2 = Res_DS_CBL(c1, c_, 1, 1, shortcut, 0.5)
        self.b3 = Res_DS_CBL(c_, c_, 3, 1, shortcut, 0.5)
        self.b4 = Res_DS_CBL(c_, c_, 3, 1, shortcut, 0.5)
        # self.b5 = Res_DS_CBL(4*c2, 2*c2, 1, shortcut, 0.5)

    def forward(self, x):
        out_1 = self.b1(x)
        out_2_1 = self.b2(x)
        out_2_2 = self.b3(out_2_1)
        out_2_3 = self.b4(out_2_2)
        out_cat = torch.cat((out_1, out_2_1, out_2_2, out_2_3), 1)  # (B, c2+c2+c2+c2, H, W)
        # out = self.b5(out_cat)  # (B, 4*C2, H, W) => (B, 2*C2, H, W)
        return out_cat
...
# model = PConv(3, 2, 'slicing')

# #### Partial Conv end  ####


# ####  PartialCBL  ####
class PartialCBL(nn.Module):
    """
        部分卷积模块 = PConv+1*1conv+BN+LeakyRelu
    """

    def __init__(self, c1, c2, n_div, p=None, act=True):  # ch_in, ch_out
        super(PartialCBL, self).__init__()
        hidden_dim = int(0.5 * c1)
        self.p_conv = PConv(c1, n_div, 'split_cat')  # for training/inference
        self.conv1 = nn.Conv2d(c1, hidden_dim, 1, 1, autopad(1, p), groups=1)  # 引入1*1 PW卷积，改变通道数
        self.bn = nn.BatchNorm2d(hidden_dim)
        self.act = nn.Mish() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.conv2 = nn.Conv2d(hidden_dim, c2, 1, 1, autopad(1, p), groups=1)  # 引入1*1 PW卷积，改变通道数

    def forward(self, x):
        return self.conv2(self.act(self.bn(self.conv1(self.p_conv(x)))))
...
class Concat(nn.Module):
    ...
    def forward(self, x):
        ...
        # # print(out.type()) # out.type: torch.cuda.FloatTensor
        # return out
...
class Conv(nn.Module):
    ...
    # 通用卷积模块,包括1卷积、1BN、1激活,激活默认SiLU,可用变量指定,不激活时用nn.Identity()占位,直接返回输入
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
