...
class InvestedResidualV3(nn.Module):
    ''' expand + depthwise + pointwise '''

    def __init__(self, in_size, expand_size, out_size, act, se, kernel_size, stride):
        super(InvestedResidualV3, self).__init__()
        self.stride = stride
...
class SqueezeExcite(nn.Module):
    def __init__(self, in_size, se_ratio):
        super(SqueezeExcite, self).__init__()
        expand_size = max(in_size // se_ratio, 8)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_size, expand_size, kernel_size=1, bias=False),
            # nn.BatchNorm2d(expand_size),
...
#     def __init__(self, inp, oup, kernel_size=1, ratio=2, dw_size=5, stride=1, relu=True):
#         super(GhostConv_, self).__init__()
#         self.oup = oup
#         init_channels = math.ceil(oup / ratio)
#         new_channels = init_channels * (ratio - 1)
#
...
#     # Ghost convolution + BN + SiLU
#     def __init__(self, c1, c2, act=True):  # ch_in, ch_out
#         super(GhostCBS, self).__init__()
#         self.ghost_conv = GhostConv_(c1, c2)
#         self.bn = nn.BatchNorm2d(c2)
#         self.act = nn.SiLU(inplace=True) if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
#
#     def forward(self, x):
#         return self.act(self.bn(self.ghost_conv(x)))
#
#     def fuseforward(self, x):
#         return self.act(self.ghost_conv(x))
...
class GhostCBL(nn.Module):
    # Ghost convolution + BN + LeakyReLU
    def __init__(self, c1, c2, k=1, r=2, d=5, s=1, act=True):  # ch_in, ch_out
        super(GhostCBL, self).__init__()
        self.ghost_conv = Ghost_Conv(c1, c2, k, 2, 5, s)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.LeakyReLU(inplace=True) if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
...
#####  BiFPN ####
# channel split in 2 parts
class Chunk(nn.Module):
    def __init__(self, chunks=2, dimension=1, short=True):
    ...
    def forward(self, x):
        x_short, x_cbs = torch.chunk(x, self.chunks, self.d)
        if self.short:
            return x_short
        else:
            return x_cbs
...
# 两个特征图add操作
# in fact, BiFPN is an add operation, so f1 and f2 must have the same channels and size
...
class GSConv(nn.Module):
    def __init__(self, c1, c2, k=1, ratio=2, dw_size=5, s=1, act=True):  # c1, c2, kernel_size, ratio, dw_size, stride, act
        ...
        self.normal_conv = nn.Sequential(
            ...
            nn.BatchNorm2d(init_channels),
            nn.LeakyReLU(inplace=True) if act else nn.Sequential(),
        )
        ...
        # c_ = c2 // 2
        # self.cv1 = Conv(c1, c_, k, s, None, g, act)  # g:gr act：分组卷积
        # self.cv2 = Conv(c_, c_, 5, 1, None, c_, act)  # 分组为c_

    def forward(self, x):
        x1 = self.normal_conv(x)
        x2 = torch.cat((x1, self.dw_conv(x1)), 1)
        # shuffle
...
class GSCBL(nn.Module):
    # GSConv + BN + LeakyReLU
    def __init__(self, c1, c2, k=1, r=2, d=5, s=1, act=True):
        super(GSCBL, self).__init__()
        self.gs_conv = GSConv(c1, c2, k, 2, 5, s)  # c1, c2, kernel_size, ratio, dw_size, stride, act
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.LeakyReLU(inplace=True) if act is True else (act if isinstance(act, nn.Module) else nn.Identity())

    def forward(self, x):
        return self.act(self.bn(self.gs_conv(x)))
